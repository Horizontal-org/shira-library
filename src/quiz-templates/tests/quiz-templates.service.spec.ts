...
describe("QuizTemplatesService", () => {
  ...
  describe("findOne", () => {
    ...
    it("throws NotFoundQuizTemplateException for an unapproved quiz when approval is required", async () => {
    ...
    it("returns an unapproved quiz by default", async () => {
  ...
  describe("create", () => {
    ...
    it("leaves plain-text description untouched aside from trimming", async () => {
      const quiz = { id: 1, title: "New Quiz", approved: true, createdAt: new Date() }
      mockDb.values
        .mockResolvedValueOnce([{ insertId: 1 }])
        .mockResolvedValueOnce(undefined)
  ...
  describe("update", () => {
    it("leaves plain-text description untouched aside from trimming", async () => {
      ...
      expect(mockDb.set).toHaveBeenCalledWith({ description: "Score < 50% to fail" })
    })
  })
