...
describe("ReviewPublishEventService", () => {
  ...
  it("accepts a question submission and clears an existing rejection note", async () => {
    mockDb.where
      .mockResolvedValueOnce([{ id: 5, resourceType: "question_template", resourceId: "12", status: "in_review" }])
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined)

    await expect(service.review(5, { status: "accepted" })).resolves.toEqual({
      id: "5",
      status: "accepted",
      rejectedNote: null,
    })
    expect(mockDb.set).toHaveBeenNthCalledWith(1, { approved: true })
    expect(mockDb.set).toHaveBeenNthCalledWith(2, { status: "accepted", rejectedNote: null })
  })
